...
type DragState =
  | { kind: "move-region"; regionId: string; pointerId: number; start: RegionPoint; transform: RegionTransform }
  | { kind: "transform-region"; regionId: string; pointerId: number; startAngle: number; startDistance: number; transform: RegionTransform }
...
type DragStart =
  | Omit<Extract<DragState, { kind: "move-region" }>, "pointerId">
  | Omit<Extract<DragState, { kind: "transform-region" }>, "pointerId">
...
export function RegionMap({ value, frameWidth, frameHeight, onChange }: RegionMapProps) {
  ...
  const activeMovableRegion = useMemo(() => {
    if (!isRegionsMode || !value.regions || value.regions.length === 0) return null;
    return value.regions.find((r) => r.id === value.activeRegionId) ?? value.regions[0];
  }, [isRegionsMode, value.activeRegionId, value.regions]);

  const activeRegionCenter = activeMovableRegion
    ? {
        x: activeMovableRegion.transform.centerX * frameWidth,
        y: activeMovableRegion.transform.centerY * frameHeight,
      }
    : null;

  const activeRegionTransformHandle = activeMovableRegion
    ? localToFrame(1, 0, activeMovableRegion.transform, frameWidth, frameHeight)
    : null;
  ...
  const moveDrag = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    if (state.kind === "move-region") {
      onChange(
        updateMovableRegion(value, state.regionId, {
          transform: {
            ...state.transform,
    ...
    if (state.kind === "transform-region") {
      const startCenter = {
      ...
      onChange(
        updateMovableRegion(value, state.regionId, {
          transform: {
            ...state.transform,
            width: clamp(state.transform.width * scale, 0.08, 1.6),
            height: clamp(state.transform.height * scale, 0.08, 1.6),
  ...
  return (
    <div className="region-map-workspace">
      <div className="region-tools" aria-label="Region tools">
        {isRegionsMode ? (
          <>
            <button type="button" onClick={() => onChange(addMovableRegion(value))}>+ Region</button>
            {value.regions && value.regions.length > 0 && activeMovableRegion && (
              <button
                type="button"
                onClick={() => onChange(removeMovableRegion(value, activeMovableRegion.id))}
              >
                Remove region
              </button>
            )}
            {activeMovableRegion && (
              <button
                ...
                onClick={() =>
                  onChange(
                    updateMovableRegion(value, activeMovableRegion.id, {
                      transform: { centerX: 0.5, centerY: 0.5, width: 0.38, height: 0.38, rotation: 0 },
                    }),
                  )
                }
              >
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => onChange({ ...value, softness: event.target.valueAsNumber })}
          />
        </label>
      </div>
      ...
      <div className="region-map-frame">
        ...
        >
          <svg
            ...
            role="img"
            aria-label={isRegionsMode ? "Movable regions map" : `${value.rows.length} by ${value.columns.length} transformable region grid`}
            ...
            {isRegionsMode ? (
              <>
                {(value.regions ?? []).map((region, index) => {
                  ...
                  const poly = regionPolygon(region.transform, frameWidth, frameHeight);
                  const isActive = region.id === (activeMovableRegion?.id ?? "");
                  const regCenter = {
                    x: region.transform.centerX * frameWidth,
                    y: region.transform.centerY * frameHeight,
                  };

                  return (
                    <g key={region.id}>
                      <polygon
                        points={pointsAttribute(poly)}
                        fill={color}
                        fillOpacity={isActive ? 0.32 : (region.prompt ? 0.2 : 0.1)}
                        stroke={color}
                        strokeOpacity={isActive ? 0.95 : 0.7}
                        strokeWidth={Math.max(1.5, plan.softnessPixels)}
                        strokeLinejoin="round"
                        style={{ cursor: "move" }}
                        onPointerDown={(event) => {
                          onChange(setActiveMovableRegion(value, region.id));
                          beginDrag(event, {
                            kind: "move-region",
                            regionId: region.id,
                      ...
                      >
                        {(region.name || `R${index + 1}`).toUpperCase()}
                      </text>
                    </g>
                  );
                })}

                {activeMovableRegion && activeRegionCenter && activeRegionTransformHandle && (
                  <>
                    <line
                      x1={activeRegionCenter.x}
                      y1={activeRegionCenter.y}
                      x2={activeRegionTransformHandle.x}
                      y2={activeRegionTransformHandle.y}
                      className="region-map__transform-arm"
                    />
                    <circle
                      cx={activeRegionCenter.x}
                      cy={activeRegionCenter.y}
                      r={18}
                      className="region-map__move"
                      onPointerDown={(event) =>
                        beginDrag(event, {
                          kind: "move-region",
                          regionId: activeMovableRegion.id,
                          start: eventPoint(event),
                          transform: activeMovableRegion.transform,
                        })
                      }
                    />
                    <rect
                      x={activeRegionTransformHandle.x - 13}
                      y={activeRegionTransformHandle.y - 13}
                      width={26}
                      height={26}
                      rx={4}
                      className="region-map__transform"
                      transform={`rotate(45 ${activeRegionTransformHandle.x} ${activeRegionTransformHandle.y})`}
                      onPointerDown={(event) =>
                        beginDrag(event, {
                          kind: "transform-region",
                          regionId: activeMovableRegion.id,
                          startAngle: angle(eventPoint(event), activeRegionCenter),
                          startDistance: Math.max(1, distance(eventPoint(event), activeRegionCenter)),
                          transform: activeMovableRegion.transform,
                        })
                      }
                    />
