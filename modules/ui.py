...
from modules import gradio_extensions, layered_canvas, sd_schedulers  # noqa: F401
        ...
        with gr.Tab("Generation", id="img2img_generation") as img2img_generation_tab, ResizeHandleRow(equal_height=False):
            with ExitStack() as stack:
                ...
                for category in ordered_ui_categories():
                    ...
                    if category == "image":
                        img2img_selected_tab = gr.Radio(
                            choices=["Img2img", "Inpaint"],
                            value="Img2img",
                            type="index",
                            label="Workflow",
                            elem_id="mode_img2img",
                        )
                        layered_editor = gr.ImageEditor(
                            label="Source, paint, and mask",
                            type="pil",
                            image_mode="RGBA",
                            height=512,
                        ...
                        with gr.Row(variant="compact", elem_id="img2img_layer_actions"):
                            clear_paint_button = gr.Button("Clear paint")
                            clear_mask_button = gr.Button("Clear mask")
            ...
            img2img_selected_tab.change(
                fn=lambda tab: gr.update(visible=tab == 1),
                inputs=[img2img_selected_tab],
                outputs=[inpaint_controls],
                show_progress=False,
            )
            ...
            toprow.button_interrogate.click(
                fn=lambda editor: interrogate(layered_canvas.source_and_paint(editor)),
                **interrogate_args,
            )

            toprow.button_deepbooru.click(
                fn=lambda editor: interrogate_deepbooru(layered_canvas.source_and_paint(editor)),
                **interrogate_args,
            )
            ...
            parameters_copypaste.add_paste_fields("img2img", layered_editor, img2img_paste_fields, override_settings)
            parameters_copypaste.add_paste_fields("inpaint", layered_editor, img2img_paste_fields, override_settings)
            parameters_copypaste.register_paste_params_button(parameters_copypaste.ParamBinding(
